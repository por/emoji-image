...
mkdirp.sync(__dirname + '/dist/images');
  ...
  for (let i = 0; i < font.numGlyphs; i++) {
    ...
    if (image && strings.length > 0) {
      ...
      for (let s of strs) {
        ...
        files.push(name);
        fs.writeFileSync(__dirname + '/dist/images/' + name + '.png', image.data);
      }
    }
  }

  // write the generated regex to a file to be included by the runtime module
  fs.writeFileSync(__dirname + '/dist/assets.js',
    '/* AUTOGENERATED. DO NOT EDIT. */\n' +
    'module.exports = {\n' + files.map(file => {
      return '"' + file + '"' + ': require("./images/160/' + file + '.png")';
    }).join(',\n')) + '}';
}
